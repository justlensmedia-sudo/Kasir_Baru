...
const { query, run, exec } = require('../src/config/database');

const restoreDb = async () => {
  console.log('==================================================');
  console.log('🚀 EKSEKUSI RESTORASI DARURAT DATABASE PRODUKSI SERVER');
  console.log('==================================================');

  // 1. Inisialisasi skema tabel jika belum ada (tanpa menghapus tabel users)
  await autoInitDb();

  // 2. Baca file backup Excel terbaru
  ...
  console.log(`📄 Menggunakan file cadangan Excel: ${backupFile}`);

  // 3. Restore Suppliers (Pemasok)
  ...
  // 5. Restore Master & Outsource Products
  console.log('🏷️ Restoring Master Barang & Outsource Products...');
  ...
  // Impor data dari Excel jika tersedia
  if (fs.existsSync(backupFile)) {
    try {
      ...
      console.warn('⚠️ Peringatan impor Excel:', e.message);
    }
  }

  // 6. Restore Materials (Stok Bahan)
  ...
  console.log('==================================================');
  console.log('✅ RESTORASI DATA DATABASE PRODUKSI SELESAI 100%!');
  ...
  console.log(`  - 👤 Akun Pengguna (Users Safe): ${userCount[0].c}`);
  console.log('==================================================');

  // Sinkronisasi salinan database untuk nama file alternatif (database.sqlite, justlens_prod.sqlite)
  ...
  if (fs.existsSync(srcDb)) {
    ...
    for (const target of targets) {
      try {
        ...
        console.log(`✓ Synchronized DB file: ${target}`);
      } catch (e) {
        console.warn(`⚠️ Gagal menyalin ke ${target}:`, e.message);
      }
    }
  }
};
