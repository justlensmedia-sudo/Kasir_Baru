...
(async () => {
  try {
    ...
    app.listen(PORT, HOST, () => {
      ...
      // Auto open browser in Windows when launched directly in interactive desktop session
      const isInteractive = process.stdout && process.stdout.isTTY && !process.env.IS_SERVICE;
      if (process.platform === 'win32' && isInteractive) {
        exec(`start http://localhost:${PORT}`, (err) => {
          if (err) console.log(`Silakan buka http://localhost:${PORT} di browser Anda.`);
        });
      }
    });
  } catch (err) {
    ...
    // Pause window on error ONLY if running in interactive terminal session
