...
const DEFAULT_SERVER_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000/api';
...
export const getServerUrl = () => {
  ...
  const stored = localStorage.getItem('justlens_server_url');

  if (isTestEnv) {
    const testUrl = import.meta.env.VITE_API_URL || 'http://localhost:5001/api';
    if (!stored || stored.includes(':5000')) {
      localStorage.setItem('justlens_server_url', testUrl);
      return testUrl;
    }
  }
